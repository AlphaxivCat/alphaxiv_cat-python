...
from alphaxiv_cat import AlphaxivCat, AsyncAlphaxivCat, DefaultAioHttpClient
from alphaxiv_cat._utils import is_dict
...
logging.getLogger("alphaxiv_cat").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[AlphaxivCat]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with AlphaxivCat(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncAlphaxivCat]:
